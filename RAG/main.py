...
def embed_query(query):
    """Embeds a query using the Gemini API."""
...
# Functions to handle Qdrant interactions
def setup_qdrant_client(url):
    """Sets up the Qdrant client."""
...
def insert_points(client, collection_name, results, texts):
    """Inserts points into the Qdrant collection."""
    points = [
        PointStruct(
            id=idx,
...
def search_collection(client, collection_name, query_vector):
    """Searches the Qdrant collection with the embedded query."""
...
# Prepare texts from the dataframe
...
if GOOGLE_API_KEY:
    ...
    client = setup_qdrant_client(url="http://localhost:6333")
    create_collection(client, collection_name)

    # Insert points into the Qdrant collection
    insert_points(client, collection_name, results, texts)
    ...
    if query:
        ...
        # Search the Qdrant collection with the embedded query
        search_results = search_collection(client, collection_name, query_vector)
